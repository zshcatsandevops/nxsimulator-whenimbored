...
import pygame, pygame.freetype, math, random, datetime, json, os
...
# Default theme
THEMES = {
    "dark":  {"BG":(20,20,25),"ACC":(0,200,255),"JOY_L":(0,120,200),"JOY_R":(255,59,48)},
    "light": {"BG":(230,230,240),"ACC":(255,90,0),"JOY_L":(180,180,180),"JOY_R":(220,220,220)},
    "synth": {"BG":(10,0,20),"ACC":(255,0,200),"JOY_L":(80,0,255),"JOY_R":(255,0,120)},
}
theme_name="dark"
BG_COLOR,ACCENT_COLOR,JOYCON_L,JOYCON_R=(THEMES[theme_name][k] for k in ("BG","ACC","JOY_L","JOY_R"))
...
# --- STATE -------------------------------------------------
def load_state():
    if os.path.exists("switchone_save.json"):
        with open("switchone_save.json","r") as f:
            return json.load(f)
    return {"brightness":1.0,"volume":0.7,"theme":"dark"}
save=load_state()
brightness,volume,theme_name=save["brightness"],save["volume"],save["theme"]
BG_COLOR,ACCENT_COLOR,JOYCON_L,JOYCON_R=(THEMES[theme_name][k] for k in ("BG","ACC","JOY_L","JOY_R"))

current_screen="home"; locked=False; selected=None; hovered=None
album_imgs=[]; trophies=["🎖️ First Launch","🏆 Mario Kart Played","⭐ 100% Brightness"]
...
def save_state():
    with open("switchone_save.json","w") as f:
        json.dump({"brightness":brightness,"volume":volume,"theme":theme_name},f)
...
def draw_home(s):
    for i,(e,l,_) in enumerate(ICONS):
        r=get_icon_rect(i)
        if hovered==i:
            glow=pygame.Surface((70,70),pygame.SRCALPHA);glow.fill((255,255,255,60))
            s.blit(glow,(r.x-SCREEN_X,r.y-SCREEN_Y))
        if selected==i: pygame.draw.rect(s,ACCENT_COLOR,(r.x-SCREEN_X,r.y-SCREEN_Y,70,70),3,12)
...
def draw_themes(s):
    ...
    for name in THEMES.keys():
        ...
        FONT_L.render_to(s,(30,y+6),name.upper(),(0,0,0))
        y+=40
...
def draw_screen():
    ...
    draw_rr(screen,bezel,(15,15,15),BEZEL_RADIUS)
    surf=pygame.Surface((SCREEN_W,SCREEN_H),pygame.SRCALPHA)
    surf.fill(THEMES[theme_name]["BG"])
...
def draw_hud():
    pygame.draw.rect(screen,(10,10,10),(0,0,WIN_W,50))
...
def draw_bottom():
    y=WIN_H-25
    pygame.draw.rect(screen,(10,10,10),(0,WIN_H-50,WIN_W,50))
    ...
    pygame.draw.rect(screen,(60,60,60),(60,WIN_H-35,100,20),border_radius=10)
...
while running:
    ...
    for e in pygame.event.get():
        if e.type==pygame.QUIT: save_state(); running=False
        elif e.type==pygame.KEYDOWN:
            if e.key==pygame.K_ESCAPE:
                if current_screen!="home": current_screen="home"; selected=None; fade_transition()
            ...
            if current_screen=="home":
                ...
                y=60
                for name in THEMES.keys():
                    if pygame.Rect(20,y,200,30).collidepoint(mx,my):
                        theme_name=name
                        BG_COLOR,ACCENT_COLOR,JOYCON_L,JOYCON_R=(THEMES[name][k] for k in ("BG","ACC","JOY_L","JOY_R"))
                        fade_transition(); save_state()
                    y+=40
